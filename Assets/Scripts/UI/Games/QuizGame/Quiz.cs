...
namespace UI.Games.QuizGame
{
    public class Quiz : MonoBehaviour
    {
        ...
        private Subject<string> _onComplete = new();
        ...
        private void Start()
        {
            submitButton.OnClickAsObservable().Subscribe(_ => OnSubmitButtonClick()).AddTo(_disposable);
            backButton.OnClickAsObservable().Subscribe(_ => gameObject.SetActive(false)).AddTo(_disposable);;
        }
        ...
        public void InitQuiz(QuizData quiz,bool alreadyComplete)
        {
            ...
            for (var i = 0; i < answers.Count; i++)
            {
                ...
                answer.OnSelected.Subscribe(OnAnswerSelected).AddTo(_disposable);;
            }
            submitButton.interactable = false;
        }
